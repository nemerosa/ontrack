...
* <<installation-docker-db-transient,test mode / transient database>>
...
For example, to specify a <<installation-docker-db-server,H2 server database>>:

[source,bash]
----
docker run \
   ...
   --env "ONTRACK_ARGS=--spring.datasource.url=jdbc:h2:tcp://localhost:9082/ontrack;MODE=MYSQL"
   ...
----

[[installation-docker-db-transient]]
==== Test mode / transient database

By running without further option, Ontrack will create a in-memory transient
H2 database instance. When the container is stopped, the data is gone.

While this is fine for test purpose, this is of course not suitable
for development and even less for production purpose.

[[installation-docker-db-volume]]
==== Database persisted in volume

To set the database on the `/var/ontrack/data` volume, use the additional
`ONTRACK_ARGS`:
...
docker run \
   ...
   --env "ONTRACK_ARGS=--spring.datasource.url=jdbc:h2:/var/ontrack/data/database/data;MODE=MYSQL;DB_CLOSE_ON_EXIT=FALSE;DEFRAG_ALWAYS=TRUE"
   ...
----
