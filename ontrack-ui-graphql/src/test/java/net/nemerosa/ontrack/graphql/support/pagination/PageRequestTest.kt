...
class PageRequestTest {
    ...
    fun `No previous page when starting from the origin`() {
        assertNull(PageRequest(0, 20).previous(40))
    }

    @Test
    fun `Full previous page`() {
        assertNotNull(PageRequest(10, 10).previous(40)) {
    ...
    fun `Offset previous page`() {
        assertNotNull(PageRequest(15, 10).previous(40)) {
    ...
    fun `Reduced previous page`() {
        assertNotNull(PageRequest(7, 10).previous(40)) {
            assertEquals(0, it.offset)
    ...
    fun `No next page when being at the end`() {
        assertNull(PageRequest(20, 20).next(40))
    }

    @Test
    fun `No next page when being after the end`() {
        assertNull(PageRequest(30, 20).next(40))
    }

    @Test
    fun `No next page when being beyond the end`() {
        assertNull(PageRequest(40, 20).next(40))
    }

    @Test
    fun `Full next page`() {
        assertNotNull(PageRequest(10, 10).next(40)) {
    ...
    fun `Offset next page`() {
        assertNotNull(PageRequest(15, 10).next(40)) {
    ...
    fun `Reduced next page`() {
        assertNotNull(PageRequest(26, 10).next(40)) {
